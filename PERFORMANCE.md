Today is 10th March 2026 and I am creating the most performant, fastest, qualityful, unlimited, free local AI in Rust. Currently the AI is running correctly with this code but this code is not production ready as the current AI is sometimes cutting off its response or not acting right and also has some performance issue. Please make this code professionally ready with professional CLI colors, optimize it so that it runs the fullest on even low-end devices correctly, and make this production code:
```rust
use anyhow::Result;
