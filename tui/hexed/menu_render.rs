...
impl Menu {
    pub fn render_in_area(&mut self, area: Rect, buf: &mut Buffer, theme_mode: &crate::tui::theme::ThemeVariant) {
        ...
        // Inner area - the border takes 1 char on each side
        let inner_area = Rect {
            x: content_area.x + 1,
            y: content_area.y + 1,
        ...
        let visible_items = inner_area.height as usize;
        ...
        for idx in self.scroll_offset..end_idx {
            ...
            let item_text = if description == "TOGGLE_MODE" {
                ...
                format!("{} {}", title, mode_indicator)
            } else if !description.is_empty() && description != "TOGGLE_MODE" && description != "TOGGLE_RECORDING" {
                let left_part = title.to_string();
                let right_part = description;
                let available_width = line_width.saturating_sub(right_part.len() + 4);
            ...
            let line_text = if item_text.len() > line_width.saturating_sub(2) {
                let truncate_at = line_width.saturating_sub(5);
            ...
            // Create line with exact width - pad with spaces and add 1 space on each side
            let padded_line = format!(" {:<width$} ", line_text, width = line_width.saturating_sub(2));
            let final_line = if padded_line.len() > line_width {
                padded_line[..line_width].to_string()
            } else {
                padded_line
            };
            
            lines.push(Line::from(Span::styled(final_line, style)));
        }

        // Fill remaining lines
        let items_shown = end_idx - self.scroll_offset;
        for _ in items_shown..visible_items {
            let empty_line = format!(" {:<width$} ", "", width = line_width.saturating_sub(2));
            let final_line = if empty_line.len() > line_width {
                empty_line[..line_width].to_string()
            } else {
                empty_line
            };
            lines.push(Line::from(Span::styled(final_line, Style::default().bg(content_bg))));
        }
