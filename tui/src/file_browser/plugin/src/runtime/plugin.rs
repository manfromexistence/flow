...
use fb_binding::{Composer, ComposerGet, ComposerSet, FileRef, cached_field};
use fb_config::YAZI;
...
// --- Fetcher
struct Fetcher {
	inner: &'static fb_config::plugin::Fetcher,

	v_cmd: Option<Value>,
}

impl Fetcher {
	pub fn new(inner: &'static fb_config::plugin::Fetcher) -> Self { Self { inner, v_cmd: None } }
}
...
// --- Spotter
struct Spotter {
	inner: &'static fb_config::plugin::Spotter,

	v_cmd: Option<Value>,
}

impl Spotter {
	pub fn new(inner: &'static fb_config::plugin::Spotter) -> Self { Self { inner, v_cmd: None } }
}
...
// --- Preloader
struct Preloader {
	inner: &'static fb_config::plugin::Preloader,

	v_cmd: Option<Value>,
}

impl Preloader {
	pub fn new(inner: &'static fb_config::plugin::Preloader) -> Self { Self { inner, v_cmd: None } }
}
...
// --- Previewer
struct Previewer {
	inner: &'static fb_config::plugin::Previewer,

	v_cmd: Option<Value>,
}

impl Previewer {
	pub fn new(inner: &'static fb_config::plugin::Previewer) -> Self { Self { inner, v_cmd: None } }
}
