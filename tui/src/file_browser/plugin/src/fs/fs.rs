...
use fb_binding::{Cha, Composer, ComposerGet, ComposerSet, Error, File, SizeCalculator, Url, UrlRef, deprecate};
use fb_config::Pattern;
use fb_fs::{mounts::PARTITIONS, provider::{Attrs, DirReader, FileHolder}};
use fb_shared::url::{UrlCow, UrlLike};
use fb_vfs::{VfsFile, provider};
...
fn access(lua: &Lua) -> mlua::Result<Function> {
	lua.create_function(|_, ()| Ok(fb_binding::Access::default()))
}

fn calc_size(lua: &Lua) -> mlua::Result<Function> {
	lua.create_async_function(|lua, url: UrlRef| async move {
		let it = if let Some(path) = url.as_local() {
			fb_fs::provider::local::SizeCalculator::new(path).await.map(SizeCalculator::Local)
		} else {
			fb_vfs::provider::SizeCalculator::new(&*url).await.map(SizeCalculator::Remote)
		};
...
fn read_dir(lua: &Lua) -> mlua::Result<Function> {
	lua.create_async_function(|lua, (dir, options): (UrlRef, Table)| async move {
		...
		while let Ok(Some(next)) = it.next().await {
			...
			let file = if !resolve {
				fb_fs::File::from_dummy(url, next.file_type().await.ok())
			} else if let Ok(cha) = next.metadata().await {
				fb_fs::File::from_follow(url, cha).await
			} else {
				fb_fs::File::from_dummy(url, next.file_type().await.ok())
			};
...
fn unique(lua: &Lua) -> mlua::Result<Function> {
	lua.create_async_function(|lua, (r#type, url): (mlua::String, UrlRef)| async move {
		let result = match &*r#type.as_bytes() {
			b"dir" => fb_vfs::unique_file(url.clone(), true).await,
			b"file" => fb_vfs::unique_file(url.clone(), false).await,
...
fn unique_name(lua: &Lua) -> mlua::Result<Function> {
	lua.create_async_function(|lua, url: UrlRef| async move {
		...
		match fb_vfs::unique_name(url.clone(), async { false }).await {
			Ok(u) => Url::new(u).into_lua_multi(&lua),
			Err(e) => (Value::Nil, Error::Io(e)).into_lua_multi(&lua),
		}
	})
