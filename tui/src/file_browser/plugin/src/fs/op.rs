...
use fb_binding::{Cha, File, Id, Path, Url};

pub(super) struct FilesOp(fb_fs::FilesOp);
...
impl FilesOp {
	...
	pub(super) fn done(_: &Lua, t: Table) -> mlua::Result<Self> {
		...
		Ok(Self(fb_fs::FilesOp::Done(url.into(), *cha, *id)))
	}
