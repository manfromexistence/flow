...
use tracing::error;
use fb_binding::{Error, File, elements::{Rect, Renderable}};
use fb_config::LAYOUT;
use fb_dds::Sendable;
use fb_parser::{app::PluginOpt, mgr::{PreviewLock, UpdatePeekedOpt}};
use fb_proxy::{AppProxy, MgrProxy};
use fb_shared::{event::Action, pool::Symbol};
...
fn peek_sync(action: &'static Action, file: fb_fs::File, mime: Symbol<str>, skip: usize) {
...
fn peek_error(file: fb_fs::File, mime: Symbol<str>, skip: usize, error: anyhow::Error) {
	...
	MgrProxy::update_peeked(UpdatePeekedOpt {
		lock: PreviewLock {
			...
			area: area.into(),
			data: vec![
				Renderable::Clear(fb_binding::elements::Clear { area: area.into() }),
