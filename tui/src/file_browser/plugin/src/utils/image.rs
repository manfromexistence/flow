...
use fb_adapter::{ADAPTOR, Image};
use fb_binding::{Error, ImageInfo, UrlRef, elements::Rect};
use fb_fs::FsUrl;
use fb_shared::url::{AsUrl, UrlLike};
...
impl Utils {
	pub(super) fn image_info(lua: &Lua) -> mlua::Result<Function> {
		lua.create_async_function(|lua, url: UrlRef| async move {
			...
			match fb_adapter::ImageInfo::new(path).await {
