...
impl Utils {
	...
	pub(super) fn uid(lua: &Lua) -> mlua::Result<Function> {
		use uzers::Users;
		lua.create_function(|_, ()| Ok(fb_shared::USERS_CACHE.get_current_uid()))
	}

	#[cfg(unix)]
	pub(super) fn gid(lua: &Lua) -> mlua::Result<Function> {
		use uzers::Groups;
		lua.create_function(|_, ()| Ok(fb_shared::USERS_CACHE.get_current_gid()))
	}

	#[cfg(unix)]
	pub(super) fn user_name(lua: &Lua) -> mlua::Result<Function> {
		use uzers::Users;
		use fb_shared::USERS_CACHE;
	...
	pub(super) fn host_name(lua: &Lua) -> mlua::Result<Function> {
		lua.create_function(|lua, ()| fb_shared::hostname().map(|s| lua.create_string(s)).transpose())
	}
}
