...
use fb_config::{YAZI, plugin::{Fetcher, Preloader}};
use fb_parser::{app::PluginOpt, tasks::ProcessOpenOpt};
use fb_shared::{CompletionToken, Id, Throttle, url::{UrlBuf, UrlLike}};
...
impl Scheduler {
	...
	pub async fn fetch_mimetype(&self, targets: Vec<fb_fs::File>) -> bool {
		let mut wg = vec![];
	...
	pub fn preload_paged(&self, preloader: &'static Preloader, target: &fb_fs::File) {
		let mut ongoing = self.ongoing.lock();
