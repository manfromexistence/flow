...
impl Deref for Core {
	type Target = fb_core::Core;

	fn deref(&self) -> &Self::Target { &self.inner }
}

impl Core {
	pub(super) fn make(inner: &fb_core::Core) -> mlua::Result<AnyUserData> {
	...
	fn add_methods<M: mlua::UserDataMethods<Self>>(methods: &mut M) {
		methods.add_meta_method_mut(MetaMethod::Index, |lua, me, key: mlua::String| {
			...
			Ok(match &*key.as_bytes() {
				...
				b"layer" => {
					reuse!(layer, Ok::<_, mlua::Error>(fb_binding::Layer::from(me.layer())))
				}
