...
use fb_binding::{Range, Style, cached_field};
use fb_config::THEME;
use fb_shared::{path::AsPath, url::UrlLike};
...
pub(super) struct File {
	idx:    usize,
	folder: PtrCell<fb_core::tab::Folder>,
	tab:    PtrCell<fb_core::tab::Tab>,
...
impl Deref for File {
	type Target = fb_fs::File;

	fn deref(&self) -> &Self::Target { &self.folder.files[self.idx] }
}

impl AsRef<fb_fs::File> for File {
	fn as_ref(&self) -> &fb_fs::File { self }
}

impl File {
	pub(super) fn make(
		idx: usize,
		folder: &fb_core::tab::Folder,
		tab: &fb_core::tab::Tab,
...
impl UserData for File {
	fn add_fields<F: UserDataFields<Self>>(fields: &mut F) {
		fb_binding::impl_file_fields!(fields);
		cached_field!(fields, bare, |_, me| Ok(fb_binding::File::new(&**me)));
	...
	fn add_methods<M: UserDataMethods<Self>>(methods: &mut M) {
		fb_binding::impl_file_methods!(methods);

		methods.add_method("icon", |_, me, ()| {
			use fb_binding::Icon;
			// TODO: use a cache
			Ok(fb_config::THEME.icon.matches(me, me.is_hovered()).map(Icon::from))
		});
		...
		methods.add_method("mime", |lua, me, ()| {
			lua.named_registry_value::<AnyUserData>("cx")?.borrow_scoped(|core: &fb_core::Core| {
