...
use fb_binding::{SER_OPT, cached_field};

use super::{Lives, PtrCell};

pub(super) struct TaskSnap {
	inner: PtrCell<fb_scheduler::TaskSnap>,
...
impl Deref for TaskSnap {
	type Target = fb_scheduler::TaskSnap;

	fn deref(&self) -> &Self::Target { &self.inner }
}

impl TaskSnap {
	pub(super) fn make(inner: &fb_scheduler::TaskSnap) -> mlua::Result<AnyUserData> {
