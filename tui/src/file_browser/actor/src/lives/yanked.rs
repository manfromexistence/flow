...
use fb_binding::{Iter, get_metatable};

use super::{Lives, PtrCell};

pub(super) struct Yanked {
	inner: PtrCell<fb_core::mgr::Yanked>,
	iter:  AnyUserData,
}

impl Deref for Yanked {
	type Target = fb_core::mgr::Yanked;

	fn deref(&self) -> &Self::Target { &self.inner }
}

impl Yanked {
	pub(super) fn make(inner: &fb_core::mgr::Yanked) -> mlua::Result<AnyUserData> {
		let inner = PtrCell::from(inner);

		Lives::scoped_userdata(Self {
			inner,
			iter: Lives::scoped_userdata(Iter::new(
				inner.as_static().iter().map(fb_binding::Url::new),
				Some(inner.len()),
			))?,
		})
	}
