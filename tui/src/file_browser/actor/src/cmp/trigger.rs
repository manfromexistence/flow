...
use anyhow::Result;
use fb_fs::{path::clean_url, provider::{DirReader, FileHolder}};
use fb_macro::{act, render, succ};
use fb_parser::cmp::{CmpItem, ShowOpt, TriggerOpt};
use fb_proxy::CmpProxy;
use fb_shared::{AnyAsciiChar, BytePredictor, data::Data, natsort, path::{AsPath, PathBufDyn, PathLike}, scheme::{SchemeCow, SchemeLike}, strand::{AsStrand, StrandLike}, url::{UrlBuf, UrlCow, UrlLike}};
use fb_vfs::provider;
...
mod tests {
	...
	fn compare(s: &str, parent: &str, child: &str) {
		...
		if let Ok(u) = p.try_strip_prefix(fb_fs::CWD.load().as_ref()) {
