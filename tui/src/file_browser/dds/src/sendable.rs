...
use fb_shared::{data::{Data, DataKey}, replace_cow};
...
impl Sendable {
	pub fn value_to_data(lua: &Lua, value: Value) -> mlua::Result<Data> {
		Ok(match value {
			...
			Value::UserData(ud) => match ud.type_id() {
				Some(t) if t == TypeId::of::<fb_binding::Url>() => {
					Data::Url(ud.take::<fb_binding::Url>()?.into())
				}
				Some(t) if t == TypeId::of::<fb_binding::Path>() => {
					Data::Path(ud.take::<fb_binding::Path>()?.into())
				}
				Some(t) if t == TypeId::of::<fb_binding::Id>() => {
					Data::Id(**ud.borrow::<fb_binding::Id>()?)
				}
				Some(t) if t == TypeId::of::<fb_fs::FilesOp>() => {
					Data::Any(Box::new(ud.take::<fb_fs::FilesOp>()?))
				}
				Some(t) if t == TypeId::of::<fb_parser::mgr::UpdateYankedIter>() => {
					Data::Any(Box::new(ud.take::<fb_parser::mgr::UpdateYankedIter>()?.into_opt(lua)?))
				}
				Some(t) if t == TypeId::of::<fb_binding::ChordCow>() => {
					Data::Any(Box::new(ud.take::<fb_binding::ChordCow>()?))
				}
	...
	pub fn data_to_value(lua: &Lua, data: Data) -> mlua::Result<Value> {
		Ok(match data {
			...
			Data::Url(u) => fb_binding::Url::new(u).into_lua(lua)?,
			Data::Path(u) => fb_binding::Path::new(u).into_lua(lua)?,
			...
			Data::Any(b) => {
				...
				try_cast!(|v: fb_fs::FilesOp| lua.create_any_userdata(v));
				try_cast!(|v: fb_parser::mgr::UpdateYankedOpt| v.into_lua(lua));
				try_cast!(|v: fb_binding::ChordCow| v.into_lua(lua));
	...
	pub fn data_to_value_ref(lua: &Lua, data: &Data) -> mlua::Result<Value> {
		Ok(match data {
			...
			Data::Id(i) => fb_binding::Id(*i).into_lua(lua)?,
			Data::Url(u) => fb_binding::Url::new(u).into_lua(lua)?,
			Data::Path(u) => fb_binding::Path::new(u).into_lua(lua)?,
			Data::Bytes(b) => Value::String(lua.create_string(b)?),
			Data::Any(b) => {
...
impl Sendable {
	fn value_to_key(value: Value) -> mlua::Result<DataKey> {
		Ok(match value {
			...
			Value::UserData(ud) => match ud.type_id() {
				Some(t) if t == TypeId::of::<fb_binding::Url>() => {
					DataKey::Url(ud.take::<fb_binding::Url>()?.into())
				}
				Some(t) if t == TypeId::of::<fb_binding::Path>() => {
					DataKey::Path(ud.take::<fb_binding::Path>()?.into())
				}
				Some(t) if t == TypeId::of::<fb_binding::Id>() => {
					DataKey::Id(**ud.borrow::<fb_binding::Id>()?)
				}
				_ => Err(format!("unsupported userdata included: {ud:?}").into_lua_err())?,
			},
			Value::Error(_) => Err("error is not supported".into_lua_err())?,
	...
	fn key_to_value(lua: &Lua, key: DataKey) -> mlua::Result<Value> {
		match key {
			...
			DataKey::Url(u) => fb_binding::Url::new(u).into_lua(lua),
			DataKey::Path(u) => fb_binding::Path::new(u).into_lua(lua),
	...
	fn key_to_value_ref(lua: &Lua, key: &DataKey) -> mlua::Result<Value> {
		Ok(match key {
			...
			DataKey::Id(i) => fb_binding::Id(*i).into_lua(lua)?,
			DataKey::Url(u) => fb_binding::Url::new(u).into_lua(lua)?,
			DataKey::Path(u) => fb_binding::Path::new(u).into_lua(lua)?,
