...
use fb_shared::url::{AsUrl, Url, UrlCow};
...
fn cue(buf: &mut Buf, s: impl AsRef<OsStr>) {
	#[cfg(unix)]
	buf.extend(fb_shared::shell::escape_os_str(s.as_ref()).as_bytes());
	#[cfg(windows)]
	buf.extend(fb_shared::shell::escape_os_str(s.as_ref()).encode_wide());
}
