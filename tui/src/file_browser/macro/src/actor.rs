...
macro_rules! act {
	(@pre $layer:ident : $name:ident, $cx:ident, $opt:ident) => {
		if let Some(hook) = <act!($layer:$name) as fb_actor::Actor>::hook($cx, &$opt) {
			<act!(core:preflight)>::act($cx, (hook, fb_dds::spark!($layer:$name, $opt))).map(|spark| spark.try_into().unwrap())
		} else {
			Ok($opt)
		}
	};
	(@impl $layer:ident : $name:ident, $cx:ident, $opt:ident) => {{
		...
		let result = match act!(@pre $layer:$name, $cx, $opt) {
			Ok(opt) => <act!($layer:$name) as fb_actor::Actor>::act($cx, opt),
			Err(e) => Err(e),
		};
	...
	($layer:ident : $name:ident, $cx:ident, $action:expr) => {
		<act!($layer:$name) as fb_actor::Actor>::Options::try_from($action)
			.map_err(anyhow::Error::from)
			.and_then(|opt| act!(@impl $layer:$name, $cx, opt))
	};
	($layer:ident : $name:ident, $cx:ident) => {
		act!($layer:$name, $cx, <<act!($layer:$name) as fb_actor::Actor>::Options as Default>::default())
	};
	($layer:ident : $name:ident) => {
		paste::paste! { fb_actor::$layer::[<$name:camel>] }
	};
