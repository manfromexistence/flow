...
use fb_binding::get_metatable;
use fb_shared::{event::ActionCow, url::UrlBufCov};

type Iter = fb_binding::Iter<
	std::iter::Map<hashbrown::hash_set::IntoIter<UrlBufCov>, fn(UrlBufCov) -> fb_binding::Url>,
	fb_binding::Url,
>;
...
impl IntoLua for UpdateYankedOpt<'_> {
	fn into_lua(self, lua: &Lua) -> mlua::Result<Value> {
		...
		let iter = Iter::new(self.urls.into_owned().into_iter().map(fb_binding::Url::new), Some(len));
