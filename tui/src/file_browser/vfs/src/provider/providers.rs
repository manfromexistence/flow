...
use tokio::sync::mpsc;
use fb_fs::{cha::Cha, provider::{Attrs, Capabilities, Provider}};
use fb_shared::{path::{AsPath, PathBufDyn}, strand::AsStrand, url::{Url, UrlBuf, UrlCow}};

#[derive(Clone)]
pub(super) enum Providers<'a> {
	Local(fb_fs::provider::local::Local<'a>),
...
impl<'a> Provider for Providers<'a> {
	...
	async fn new<'b>(url: Url<'b>) -> io::Result<Self::Me<'b>> {
		use fb_shared::scheme::SchemeKind as K;

		Ok(match url.kind() {
			K::Regular | K::Search => Self::Me::Local(fb_fs::provider::local::Local::new(url).await?),
			K::Archive => {
