...
use fb_config::vfs::{ServiceSftp, Vfs};
use fb_fs::provider::{Capabilities, DirReader, FileHolder, Provider};
use fb_sftp::fs::{Attrs, Flags};
use fb_shared::{loc::LocBuf, path::{AsPath, PathBufDyn}, pool::InternStr, scheme::SchemeKind, strand::AsStrand, url::{Url, UrlBuf, UrlCow, UrlLike}};
...
impl<'a> Provider for Sftp<'a> {
	...
	async fn copy<P>(&self, to: P, attrs: fb_fs::provider::Attrs) -> io::Result<u64>
	where
		P: AsPath,
	{
	...
	async fn create_dir(&self) -> io::Result<()> {
		...
		if let Err(fb_sftp::Error::Status(status)) = &result
			&& status.is_failure()
	...
	async fn metadata(&self) -> io::Result<fb_fs::cha::Cha> {
	...
	async fn symlink_metadata(&self) -> io::Result<fb_fs::cha::Cha> {
