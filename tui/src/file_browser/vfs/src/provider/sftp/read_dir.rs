...
use fb_fs::provider::{DirReader, FileHolder};
use fb_shared::{path::PathBufDyn, strand::StrandCow, url::{UrlBuf, UrlLike}};

use super::{Cha, ChaMode};

pub struct ReadDir {
	pub(super) dir:    Arc<UrlBuf>,
	pub(super) reader: fb_sftp::fs::ReadDir,
}
...
pub struct DirEntry {
	dir:   Arc<UrlBuf>,
	entry: fb_sftp::fs::DirEntry,
}

impl FileHolder for DirEntry {
	async fn file_type(&self) -> io::Result<fb_fs::cha::ChaType> {
		Ok(ChaMode::try_from(self.entry.attrs())?.0.into())
	}

	async fn metadata(&self) -> io::Result<fb_fs::cha::Cha> { Ok(Cha::try_from(&self.entry)?.0) }
