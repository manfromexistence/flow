...
use fb_fs::{FsHash64, FsHash128, FsUrl};
use fb_shared::{path::{PathLike, StripPrefixError}, scheme::{SchemeCow, SchemeLike}, strand::{StrandLike, ToStrand}, url::{AsUrl, UrlCow, UrlLike}};
...
impl Deref for Url {
	type Target = fb_shared::url::UrlBuf;

	fn deref(&self) -> &Self::Target { &self.inner }
}

impl AsUrl for Url {
	fn as_url(&self) -> fb_shared::url::Url<'_> { self.inner.as_url() }
}
...
impl From<Url> for fb_shared::url::UrlBuf {
	fn from(value: Url) -> Self { value.inner }
}

impl From<Url> for fb_shared::url::UrlBufCov {
	fn from(value: Url) -> Self { Self(value.inner) }
}

impl From<Url> for fb_shared::url::UrlCow<'_> {
...
impl Url {
	pub fn new(url: impl Into<fb_shared::url::UrlBuf>) -> Self {
		Self {
			inner: url.into(),
