...
pub struct ImageInfo(fb_adapter::ImageInfo);

impl Deref for ImageInfo {
	type Target = fb_adapter::ImageInfo;

	fn deref(&self) -> &Self::Target { &self.0 }
}

impl From<fb_adapter::ImageInfo> for ImageInfo {
	fn from(value: fb_adapter::ImageInfo) -> Self { Self(value) }
}
...
// --- ImageColor
struct ImageColor(fb_adapter::ImageColor);

impl UserData for ImageColor {
	fn add_methods<M: mlua::UserDataMethods<Self>>(methods: &mut M) {
		methods.add_meta_method(MetaMethod::ToString, |_, me, ()| {
			use fb_adapter::ImageColor as C;
