...
impl From<fb_config::Style> for Style {
	fn from(value: fb_config::Style) -> Self { Self(value.into()) }
}
...
impl UserData for Style {
	fn add_methods<M: UserDataMethods<Self>>(methods: &mut M) {
		...
		methods
			.add_method("raw", |lua, me, ()| lua.to_value_with(&fb_config::Style::from(me.0), SER_OPT));
