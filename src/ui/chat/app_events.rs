...
impl ChatApp {
    pub fn handle_mouse(&mut self, mouse: crossterm::event::MouseEvent) {
        ...
        match mouse.kind {
            ...
            MouseEventKind::Down(crossterm::event::MouseButton::Left) => {
                ...
                if self.is_in_rect(x, y, self.plan_button_area) {
                    self.show_plan_modal = true;
                    self.plan_modal_list.reset();
                    app_helpers::play_sound("click");
    ...
    fn handle_key_with_focus(&mut self, key: event::KeyEvent) {
        ...
        match (key.code, key.modifiers, self.focus) {
            ...
            (KeyCode::Tab, _, _) if self.show_focus_menu => {
                // Close focus menu if already open
                self.show_focus_menu = false;
                app_helpers::play_sound("click");
            }
            (KeyCode::Tab, _, _) => {
            ...
            // Arrow keys removed - they now control animations instead of mode switching
            ...
            (_, _, _) if self.show_focus_menu => {
                self.handle_focus_menu(key);
            }
            ...
            (KeyCode::Right, KeyModifiers::NONE, Focus::Input) if self.input.content.is_empty() => {
    ...
    fn handle_focus_menu_selection(&mut self, idx: usize) {
        match idx {
            0 => {
                // Agent Mode
                self.mode = super::modes::ChatMode::Agent;
                self.last_shortcut_pressed = Some("Switched to Agent mode".to_string());
            }
            1 => {
                // Plan Mode
                self.mode = super::modes::ChatMode::Plan;
                self.last_shortcut_pressed = Some("Switched to Plan mode".to_string());
            }
            2 => {
                // Ask Mode
                self.mode = super::modes::ChatMode::Ask;
                self.last_shortcut_pressed = Some("Switched to Ask mode".to_string());
            }
            3 => {
                // Model Selector
                self.show_model_modal = true;
    ...
    fn handle_local_modal(&mut self, key: event::KeyEvent) {
        ...
        match action {
            ModalListAction::ItemSelected(idx) => {
                ...
                self.selected_local_mode = mode.clone();
                self.last_shortcut_pressed = Some(message);
                self.last_shortcut_time = Instant::now();
                self.show_local_modal = false;

                // Initialize the backend based on selection
                if let Some(llm) = &self.llm {
                    if idx == 0 {
                        // Local mode selected - ensure it's initialized
                        let llm_clone = llm.clone();
                        tokio::spawn(async move {
                            let _ = llm_clone.initialize().await;
                        });
                    }
                }
