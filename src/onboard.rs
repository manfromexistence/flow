...
async fn discover_provider_models(provider_id: &str) -> ProviderModelListing {
    ...
    let api_key = if provider_id == "github_copilot" || provider_id == "github-copilot" {
        ...
        match cli::llm::copilot::retrieve_copilot_token() {
            Some(token) => token,
            None => {
                return ProviderModelListing {
                    provider_id: provider_id.to_string(),
...
async fn ensure_turso_schema(conn: &libsql::Connection) -> Result<()> {
    ...
    conn.execute(
        "CREATE TABLE IF NOT EXISTS cliing_profiles (
            id TEXT PRIMARY KEY,
    ...
    ChatRequest {
        ...
        messages: vec![ChatMessage {
            ...
            content: MessageContent::Text(
                "hello, just tell me what is 2+2 in math nothing else".to_string(),
            ),
            name: None,
        }],
        temperature: None,
...
async fn run_cliing_flow(_args: OnboardCliArgs) -> Result<()> {
    ...
    for provider in &providers {
        if provider == "github_copilot" || provider == "github-copilot" {
            ...
            match cli::llm::ensure_github_copilot_ready_interactive().await {
                Ok(result) => {
                    eprintln!(
